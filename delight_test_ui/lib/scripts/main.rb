...
class Delight_Test
  ...
  def test()
    ...
    if !@test_cases[0].downcase.eql?("login")
      login("dgee", "N1nja123")
    end
    @test_cases.each do |test_name|
      @lead_state = test_name
...
tester = Delight_Test.new(test_cases)
tester.test()

# num_browsers = 2
# num_browsers.times {
#   tester = Delight_Test.new(["login"])
#   tester.test()
# }

write_results_to_file("lib/assets/results.json")
